...
/**
 * Access controller for the artist entity. Controls create/edit/delete access
 * for entity and fields.
 *
 * @see \Drupal\egam_artist\Entity\Artist.
 */
class ArtistAccessControlHandler extends EntityAccessControlHandler {
	
	const ADMINISTER_ARTIST_PERMISSION = 'administer artist';
		...
		switch ($operation) {
			case 'view':
				...
				if ($entity->get('moderation_state')->getString() == 'draft') {
					if ($account->hasPermission(self::ADMINISTER_ARTIST_PERMISSION)) {
	...
	protected function checkCreateAccess(AccountInterface $account, array $context, $entity_bundle = NULL) {
		return AccessResult::allowedIfHasPermission($account, self::ADMINISTER_ARTIST_PERMISSION);
	}
